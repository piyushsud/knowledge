...
\begin{itemize}
    ...
    \item Rs = \(\frac{2*pi*f_c*N}/{I_{cp}*K_{Vco}}\). N varies between 77 and 97, so let's choose 87 as the nominal value. According to the datasheet, it is best to start with 2.5 mA for the charge pump current. The VCO gain is given by 6.2 MHz/V. Therefore, \(Rs = \frac{2*3.1415*10000*87}{2.5*10^-3*6.2*10^6} = 352.658709677\) ohms. 
...
\noindent Simulated using ADISimPLL to find loop filter values, lock time ~= 500 us, PM = 60 degrees, loop BW = 10 kHz, CP current = 2.4 mA.



\begin{figure}[h]
    \includegraphics[width=\textwidth]{pll_freq_response}
\end{figure}

\begin{figure}[h]
    \includegraphics[width=\textwidth]{pll_schematic}
\end{figure}
