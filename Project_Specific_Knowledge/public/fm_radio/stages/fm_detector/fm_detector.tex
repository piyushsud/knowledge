...
\noindent where bb(t) is the baseband signal. The signal after C2 is shifted by 90 degrees due to the capacitor, and additionally shifted by \(\phi\) due to the difference between the resonant frequency of the tank circuit and the incoming FM signal. Therefore the signal after C2 is 

\[Acos((w + bb(t))t + \frac{\pi}{2} + \phi)\] 

Multiplying these two together gives 

\[Acos((w + bb(t))t) * Acos((w+bb(t))t + \frac{\pi}{2} + \phi)\] 

By the identity \(cos(a)cos(b) = \frac{1}{2}[cos(a-b) + cos (a+b)] \),

\[= \frac{A^2}{2}cos(-\pi/2 - \phi) + cos(2(w+bb(t))t + \frac{\pi}{2} + \phi)\] 

Noting that \(cos(-\pi/2 - \phi) = sin(\phi)\), 

\[= \frac{A^2}{2}sin(\phi) + cos(2(w + bb(t))t + \frac{\pi}{2} + \phi)\] 

\noindent The higher frequency cosine component can be filtered out, leaving a function whose amplitude is proportional to the phase, which in turn is proportional to \(sin(\phi)\). For small values of \(\phi\), this is approximately equal to \(\phi\).
...
\begin{itemize}
    \item For the tank circuit, the self resonant frequency is given by \( \frac{1}{2\pi \sqrt{LC}}\). If we choose L = 0.1 uH, then for a resonant frequency of 10.7 MHz, this gives us 2.21 nF. 
    ...
    \item We need a mixer that can mix down to very low frequencies, since the mono baseband is typically 50 Hz to 15 kHz. Let's use the AD831, since the output can go down to DC.
\end{itemize}
...
Here's a simulation of the fm detector working at low frequencies (at high frequencies LTSpice doesn't handle it very well) 

\begin{figure}[h]
    \includegraphics[width=\textwidth]{discriminator_sim_low_freq}
\end{figure}
