...
WAIT AT AUDIO FREQ U CANT USE 0.1 uF BYPASS CAPS! FREQ CAN BE 20 Hz!!

so output needs to be dc coupled to vcc. this won't work. need to choose a different mixer!

The AD831 seems like a good option - it says, "all ports can be DC coupled"!

\begin{itemize}
    \item For the mixer, at 10.7 MHz the frequency is sufficienctly low so we can assume the input impedance is mostly low. In fact, at this frequency PCB trace length is probably not an issue so impedance matching is not necessary. 
...
Here's a simulation of the output impedance:
